...
function TailorResumeDetailsContent() {
  ...
  useEffect(() => {
    try {
      ...
      // Clean up the review_suggestions string
      const reviewSuggestionsString = parsedData.review_suggestions.replace(/^json\n/, '').trim();
      console.log('Cleaned Review Suggestions String:', reviewSuggestionsString);

      // Parse the cleaned review suggestions JSON string
      const reviewSuggestions = JSON.parse(reviewSuggestionsString);
      console.log('Review Suggestions:', reviewSuggestions);
  ...
  const renderHighlightedText = (text: string, color: string) => {
    ...
    return (
      <div className={`p-3 rounded-md ${bgColorMap[color] || 'bg-gray-100'}`}>
        {text}
      </div>
    );
  };
                ...
                <CardContent className="space-y-6">
                  ...
                  {tailoredData.sections.new_sections && (
                    <div className="space-y-4">
                      ...
                      {tailoredData.sections.new_sections.map((section, index) => (
                        <div key={index} className="space-y-2">
                          <h4 className="font-medium">{section.title}</h4>
                          {renderHighlightedText(section.content, section.highlight_color)}
                        </div>
                      ))}
                    </div>
                  )}
